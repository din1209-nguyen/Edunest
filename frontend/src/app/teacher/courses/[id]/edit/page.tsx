...
function normalizeCurriculum(chapters: Chapter[] = []): Chapter[] {
  return chapters.map((chapter, chapterIndex) => ({
    ...
    lessons: (chapter.lessons || []).map((lesson, lessonIndex) => ({
      ...lesson,
      type: lesson.type === "pdf" ? "document" : lesson.type,
...
export default function TeacherCourseEditPage() {
  ...
  const previewStats = useMemo(() => {
    ...
    return [
      ...
      { label: "Chương", value: metrics.chapterCount > 0 ? `${metrics.chapterCount} chương` : "Chưa có" },
      { label: "Bài học", value: metrics.lessonCount > 0 ? `${metrics.lessonCount} bài học` : "Chưa có" },
      { label: "Bài tập", value: metrics.exerciseCount > 0 ? `${metrics.exerciseCount} bài tập` : "Chưa có" },
    ];
  }, [form, metrics.chapterCount, metrics.lessonCount, metrics.exerciseCount]);
  ...
  const handleCreateChapter = async () => {
    ...
    try {
      ...
      setError("");
      await chapterApi.createChapter(course._id, newChapterTitle.trim());
      setNewChapterTitle("");
  ...
  return (
    <div className="content-stack">
      ...
      {successMessage && <div className="rounded-lg border border-success/30 bg-success-light px-4 py-3 text-sm text-success">{successMessage}</div>}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Curriculum của khóa học
              </CardTitle>
              <CardDescription>Thêm chương, bài học và bài tập trực tiếp ngay trong màn hình chỉnh sửa này.</CardDescription>
            </CardHeader>
                                ...
                                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                                  <div>
                                    <div className="flex flex-wrap items-center gap-2">
                                      ...
                                      <Badge variant="secondary-light">{lesson.type}</Badge>
                                ...
                                {isEditing && (
                                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                                    ...
                                    <div className="flex items-end">
                                      <TeacherFileUploadButton
                                        ...
                                        onUploaded={(url) => {
                                          ...
                                          updateLessonForm(lesson._id, "type", "document");
                                        }}
                                        onError={setError}
                                      />
                                    </div>
                                    <Select
                                      ...
                                      onChange={(value) => updateLessonForm(lesson._id, "type", value as LessonEditorForm["type"])}
                                    />
                                    <Select
                                      label="Loại tài liệu"
                                      options={[
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="rounded-2xl border border-border bg-surface/70 p-4">
                  ...
                  <p className="mt-1 text-sm font-semibold text-foreground">{categoryLabel(course.category)}</p>
                </div>
                <div className="rounded-2xl border border-border bg-surface/70 p-4">
                ...
                <div className="rounded-2xl border border-border bg-surface/70 p-4">
                  ...
                  <p className="mt-1 text-sm font-semibold text-foreground">{currentStatus.label}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Tiến trình bổ sung nội dung
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-muted-foreground">
              {previewStats.map((item) => (
                <div key={item.label} className="flex items-center justify-between rounded-2xl border border-border bg-surface/70 px-4 py-3">
                  <span>{item.label}</span>
                  <span className="font-medium text-foreground">{item.value}</span>
                </div>
              ))}
              <div className="rounded-2xl border border-warning/20 bg-warning/10 p-4 text-warning">
                Khi course đã có đủ thông tin cơ bản, ít nhất một chương và một bài học, bạn có thể gửi duyệt ngay từ màn hình này. Bài tập hiện đã hỗ trợ builder nhiều câu hỏi và bám schema backend.
              </div>
            </CardContent>
          </Card>
        </div>
